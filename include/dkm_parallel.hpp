...
std::vector<T> closest_distance(
	...
	std::vector<std::future<T>> distance_futures;
	distance_futures.reserve(data.size());
	distances.reserve(data.size());
	for (const auto& d : data) {
		distance_futures.push_back(std::async(std::launch::async, [&]{
	...
	for (auto& f : distance_futures) {
		distances.push_back(f.get());
	}
	return distances;
}
...
std::vector<uint32_t> calculate_clusters(
	...
	std::vector<std::future<uint32_t>> cluster_futures;
	cluster_futures.reserve(data.size());
	clusters.reserve(data.size());
	for (const auto& point : data) {
		cluster_futures.push_back(std::async(std::launch::async, [&]{
			return closest_mean(point, means);
    }));
	}
	for (auto& f : cluster_futures) {
		clusters.push_back(f.get());
	}
	return clusters;
}
